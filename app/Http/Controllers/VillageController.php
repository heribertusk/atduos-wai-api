...
class VillageController extends Controller
{
    ...
    public function index(Request $request): ApiSuccessResponse|ApiErrorResponse
    {
        $filters = $request->validate([
            'name' => 'nullable|alpha|min:3|max:50|required_without:district_id',
            'district_id' => 'nullable|numeric|required_without:name',
        ]);

        $query = DB::table('reg_villages')->when($filters['name'] ?? null, function ($q, $search) {
            $q->where('name', 'like', '%'.$search.'%');
        })->when($filters['district_id'] ?? null, function ($q, $search) {
    ...
    {
        try {
            $record = DB::table('reg_villages')->where('id', $id)->first();
